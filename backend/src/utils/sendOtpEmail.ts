...
const sendOtpEmail = async (toEmail: string, name: string, otp: string) => {
  ...
  const emailTemplate = `<div style="font-family: Roboto, sans-serif; max-width: 600px; margin: auto; padding: 20px; background-color: #f9f9f9; border: 1px solid #e0e0e0; border-radius: 10px;">
    <div style="text-align: center;">
      <img src="https://daleelk-backend.vual.in/uploads/logo.png" alt="Logo" style="max-width: 150px; margin-bottom: 20px;">
    </div>
    <h2 style="color: #00125c; text-align: center;">Reset Your Password</h2>
    <p style="color: #555555;">Hi <strong>${name}</strong>,</p>
    <p style="color: #555555;">
      We received a request to reset the password for your Lr consultants account.
    ...
    <p style="color: #555555;">Thank you,<br>The Lr consultants Team</p>
    <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 20px 0;">
    <p style="font-size: 12px; color: #999999; text-align: center;">
      &copy; 2025 Lr consultants. All rights reserved.
    </p>
  </div>
`;
  ...
  try {
    await transporter.sendMail({
      from: `"Lr consultants Support" <${process.env.SMTP_USER}>`,
      to: toEmail,
      subject: `Your OTP Code - Lr consultants`,
      html: emailTemplate,
    });
  } catch (err) {
